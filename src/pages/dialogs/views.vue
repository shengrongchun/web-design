  ...
  <div class="viewsContainer">
    <el-tree class="filter-tree"
      ...
      <span class="custom-tree-node"
        ...
        <span>{{data.type==='to-router-page'?'页面/':''}}{{ data.title }}</span>
        <span>
          <el-button type="text"
        ...
        <el-form-item label="已占路径"
          ...
          <ul class="pathUl">
            <li v-for="(page,idx) in tempData.children"
                :key="idx">{{page.title}}：{{page.path}}</li>
          </ul>
        </el-form-item>
      </el-form>
      <span slot="footer"
...
export default {
  ...
  methods: {
    ...
    onNodeClick({ path, type }) {//节点点击事件
      if (type === 'to-router-page' && this.$route.fullPath !== path) {//页面点击
        this.$router.push({ path })
      }
    },
    onFilterNode(value, { type }) {//过滤节点
